...
// Fetch signal count dynamically
function useSinyalCount() {
  const { data } = useApi<{ signals: unknown[] }>("/api/signals?limit=50", { signals: [] });
  const count = data?.signals?.length ?? 0;
  return count > 0 ? String(count) : null;
}

// Fetch token count dynamically
function useScannerCount() {
  const { data } = useApi<{ total: number }>("/api/tokens?limit=1", { total: 0 });
...
export default function Sidebar() {
  ...
  const sinyalBadge = useSinyalCount();
  const scannerBadge = useScannerCount();
  ...
  const NAV_GROUPS = [
    {
      ...
      items: [
        ...
        { href: "/scanner", name: "Scanner token", badge: scannerBadge, badgeType: "neutral" as const },
    ...
    },
    {
      label: "ANALISIS",
      items: [
        { href: "/sinyal", name: "Sinyal baru", badge: sinyalBadge, badgeType: "amber" as const },
